...
class FirecrawlService {
  /// Base URL for content extraction API
  ...
  /// Extracts the full content from a web page at the given [url]
  /// Returns null if content extraction fails or if API key is not configured
  /// 
  /// This method sends a scraping request to the Firecrawl API
  /// to retrieve the page content in markdown format, which is particularly
  /// useful for extracting the full content of news articles.
  /// 
  /// References:
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true && responseData['data'] != null) {
          final String? markdown = responseData['data']['markdown'];
          if (markdown != null && markdown.isNotEmpty) {
            return _cleanContent(markdown);
          }
        }
      } else {
        if (kDebugMode) {
          print('Content extraction failed with status: ${response.statusCode}');
  ...
  Future<Article> extractArticleContent(Article article) async {
    try {
      final String? extractedContent = await extractContent(article.url);
      ...
      if (extractedContent != null && extractedContent.isNotEmpty) {
        ...
        return Article(
          ...
          content: extractedContent, // Use extracted content
        );
      }
    } catch (e) {
      if (kDebugMode) {
        print('Failed to extract content for article: ${article.title}');
        print('Error: $e');
      }
    }
  ...
  /// Checks if the content extraction service is properly configured.
  /// 
